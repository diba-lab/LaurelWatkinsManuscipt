...
%%
sessnum = 32:39;
figureName = 'Figure4a_CA1LEDstim_null.pdf';
statName = 'Figure4a_CA1LEDstim_deltaFR_null.xls';
propName = 'Figure4a_CA1LEDstim_proportions_null.xls';
fStatName = 'Figure4a_CA1LEDstim_ftest_null.xls';
%% delta FR
sessionID = [];
cellShank = [];
stimShank = [];
...
%         stimShank = [stimShank sS*ones(length(spikes.shankID),1)'];
        ...
        nullTimes = laser.(fields{uLED})(:,1)+(dur/1000); %laser off times
        %dur = round(mean(laser.(fields{uLED})(:,2)-laser.(fields{uLED})(:,1))*1000); %stim time in ms
