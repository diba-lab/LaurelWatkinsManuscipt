...
%%
sessnum = 19:25;
figureName = 'Figure3a_CA1uLEDsilence_null.pdf';
statName = 'Figure3a_CA1uLEDsilence_deltaFR_null.xls';
propName = 'Figure3a_CA1uLEDsilence_proportions_null.xls';
fStatName = 'Figure3a_CA1uLEDsilence_ftest_null.xls';
%% delta FR
sessionID = [];
cellShank = [];
stimShank = [];
...
%         qdaClass = [qdaClass cell_metrics.qdaClassification];
%         stimShank = [stimShank sS*ones(length(spikes.shankID),1)'];
        nullvalue = [nullvalue nullResp.(fields{uLED}).pvalue];
        nullResponse = [nullResponse nullResp.(fields{uLED}).laserResponse];
        
        nullTimes = laser.(fields{uLED})(:,2); %laser off times
        %dur = round(mean(laser.(fields{uLED})(:,2)-laser.(fields{uLED})(:,1))*1000); %stim time in ms
        
        for cellID = 1:length(spikes.UID)
            spikeTimes = spikes.times{cellID};
            
            [psth,trialspx] = mpsth(spikeTimes,trigTimes,'pre',-dur,'post',3*dur,'chart',0);
...
f_test = table(distance_um,PyramidalFStat,PyramidalDF,PyramidalFTest,...
    InterneuronFStat,InterneuronDF,InterneuronFTest);
